...
const Factures = () => {
  ...
  useEffect(() => {
    if (!isEditMode) {
      const savedArticles = localStorage.getItem('facture_draft_articles');
      if (savedArticles) {
        try {
  ...
  useEffect(() => {
    if (!isEditMode) {
      if (nouvelleFacture.liste_articles.length > 0) {
        localStorage.setItem('facture_draft_articles', JSON.stringify(nouvelleFacture.liste_articles));
      } else {
        localStorage.removeItem('facture_draft_articles');
      }
    }
  ...
  const filteredFactures = useMemo(() => {
    return factures.filter(facture => {
      ...
      const isRealFacture = facture.status === 'facture' || !facture.status;
      if (!isRealFacture) return false;

      if (statusFilter === 'paid') matchesStatus = (paid >= total);
      if (statusFilter === 'partial') matchesStatus = (paid > 0 && paid < total);
      if (statusFilter === 'unpaid') matchesStatus = (paid <= 0);
