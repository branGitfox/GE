...
exports.createClient = async (req, res) => {
    ...
    try {
        // Vérification des doublons (téléphone ou email valide)
        ...
        const insertResult = await new Promise((resolve, reject) => {
            connection.query(
                insertQuery,
                [nom, adresse, telephone, finalEmail],
                (error, results) => {
                    if (error) reject(error);
                    resolve(results);
                }
...
exports.updateClient = (req, res) => {
    ...
    db.query(
        ...
        (error, phoneResults) => {
            ...
            // 2. Si tout est bon, faire la mise à jour
            db.query(
                'UPDATE clients SET nom = ?, adresse = ?, telephone = ?, email = ? WHERE id = ?',
                [nom, adresse, telephone, finalEmail, id],
                (error, results) => {
                    if (error) {
                    ...
                    res.json({
                        ...
                        client: { id, nom, adresse, telephone, email: finalEmail }
                    });
                }
            );
        }
